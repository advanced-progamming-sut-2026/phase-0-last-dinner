...
public class ZombotanyView implements CommandHandler {
    ...
    public void handleCommand(String input) {
        ...
        matcher = ZombotanyCommands.START.getMatcher(input);
        if (matcher != null) {
            handleStart(matcher);
            return;
        }

        matcher = ZombotanyCommands.PLANT.getMatcher(input);
        if (matcher != null) {
            handlePlant(matcher);
            return;
        }
        ...
        matcher = ZombotanyCommands.HELP.getMatcher(input);
        if (matcher != null) {
            showCommandHelp();
            return;
        }
    ...
    private void showAvailablePlants(
        ...
        if (state.getAvailablePlants().isEmpty()) {
            System.out.println("- None");
            return;
        }

        for (PlantDefinition definition
                : state.getAvailablePlants()) {
            System.out.println(
                    "- "
                            + definition.getName()
                            + " | cost: "
