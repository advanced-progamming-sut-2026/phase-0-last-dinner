...
public class GreenhouseView implements CommandHandler {
    ...
    private CommandHandler activeShopHandler;

    @Override
    public void handleCommand(String input) {
        if (observer == null) {
            System.out.println("Greenhouse controller is not connected.");
            return;
        }

        if (activeShopHandler != null && isShopCommand(input)) {

            activeShopHandler.handleCommand(input);
            return;
        }
        ...
        matcher = GreenhouseCommands.PLANT_POT.getMatcher(input);
        ...
        matcher = GreenhouseCommands.COLLECT.getMatcher(input);
        ...
        matcher = GreenhouseCommands.GROW.getMatcher(input);
        ...
        System.out.println("Invalid greenhouse command.");
    }

    public void showGreenhouse() {
        GreenhouseStateResult state = observer.onShowGreenhouseRequested();

        if (state == null) {
            System.out.println("Greenhouse is not available.");
            return;
        }
    ...
    private void handlePlant(Matcher matcher) {
        ...
        GreenhouseActionResult result = observer.onPlantPotRequested(position);
    ...
    private void handleCollect(Matcher matcher) {
        ...
        GreenhouseActionResult result = observer.onCollectRequested(position);
    ...
    private void handleGrow(Matcher matcher) {
        ...
        GreenhouseActionResult result = observer.onGrowRequested(position);
    ...
    private void printActionResult(GreenhouseActionResult result) {
        if (result == null) {
            System.out.println("Greenhouse action failed.");
            return;
        }
    ...
    private void printGreenhouse(
        ...
        System.out.println("Coins: " + state.getCoins() + " | Diamonds: " + state.getDiamonds());
    ...
    private void printStoredBoosts(Set<String> storedBoosts) {
        if (storedBoosts == null || storedBoosts.isEmpty()) {
            System.out.println("Stored boosts: none");
            return;
        }

        System.out.println("Stored boosts: " + String.join(", ", storedBoosts));
    }
    ...
    private void handleEnterShop() {
        ...
        activeShopHandler = shopHandler;

        System.out.println("Shop opened.");

        System.out.println("Use 'shop list' or 'shop daily'.");
    }
