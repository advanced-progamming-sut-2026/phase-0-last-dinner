...
public class ZombotanyStateResult {
  ...
  private final Map<ZombieDefinition, ZombotanyTrait> zombieTraits;
  ...
  public ZombotanyStateResult(
    ...
    this.plantFoodAmount = Math.max(0, plantFoodAmount);
    ...
    this.zombieTraits =
        zombieTraits == null
            ...
            : Collections.unmodifiableMap(new LinkedHashMap<>(zombieTraits));
    this.plantGrid = copyGrid(plantGrid);
    this.zombies =
        zombies == null
            ? Collections.emptyList()
            : Collections.unmodifiableList(new ArrayList<>(zombies));
    this.collectibleSunPositions =
        collectibleSunPositions == null
            ? Collections.emptyList()
            : Collections.unmodifiableList(new ArrayList<>(collectibleSunPositions));
  ...
  public String getPlantNameAt(Position position) {
    ...
    if (rowIndex < 0 || rowIndex >= plantGrid.size()) {
      return "";
    }

    List<String> row = plantGrid.get(rowIndex);
