...
public class PlantZombieIZombieIntegration
    ...
    private PlantZombieIZombieIntegration(
    ...
    public PlantZombieIZombieIntegration(
        ...
        this.zombieFactory = Objects.requireNonNullElseGet(zombieFactory, () -> new ZombieFactory(zombieDefinitions));
    ...
    public List<ZombieDefinition> chooseAvailableZombies(
        ...
        List<ZombieDefinition> selected =
                new ArrayList<>();
        ...
        for (String alias
                : ZOMBIES_BY_STAGE[stageNumber - 1]) {
        ...
        if (selected.size()
                != IZombieStageConfig
                .AVAILABLE_ZOMBIE_COUNT) {

            throw new IllegalStateException(
                    "I, Zombie requires exactly "
                            + "five selectable zombies."
            );
        }
    ...
    public int getZombieSunCost(
        ...
        if (zombieDefinition == null
                || zombieDefinition.getType() == null) {

            return -1;
        }

        return switch (zombieDefinition.getType()) {
            case BASIC -> 50;
            case IMP -> 50;
            case ANIMAL -> 75;
            case ARMORED -> 100;
    ...
    public void spawnInitialSunProducerZombies(
        ...
        IZombieStageConfig config =
                stageGenerator.generateStage(stageNumber);

        ZombieDefinition producerDefinition =
                findSunProducerZombieDefinition();

        if (producerDefinition == null) {
            throw new IllegalStateException(
                    "No zombie definition is available "
                            + "for the sun producers."
            );
        }

        for (int userRow = 1;
             userRow <= BOARD_ROW_COUNT;
             userRow++) {

            Position position = new Position(
                    FIRST_ZOMBIE_COLUMN - 1,
                    userRow - 1
            );
            ...
            IZombieSunProducerBehavior behavior =
                    new IZombieSunProducerBehavior(
                            miniGame,
    ...
    public boolean placeZombie(
        ...
        if (!board.isInsideBoard(boardPosition)
                || isZombiePlacementBlocked(userPosition)) {
    ...
    public boolean isBrainEaten(int userRow) {
        return board != null
                && board.isBrainEatenInRow(userRow);
    }

    private void placeDefensivePlants(
            int stageNumber
    ) {
        List<PlantDefinition> candidates =
                findPlantCandidates(stageNumber);

        if (candidates.isEmpty()) {
            throw new IllegalStateException(
                    "No defensive plants are available "
                            + "for I, Zombie."
            );
        }

        int plantsPerRow = Math.min(
                5,
                stageNumber + 2
        );
    ...
    private List<PlantDefinition> findPlantCandidates(
            int stageNumber
    ) {
        List<PlantDefinition> candidates =
                new ArrayList<>();
        ...
        for (String name
                : PLANTS_BY_STAGE[stageNumber - 1]) {

            PlantDefinition definition =
                    findPlantIgnoringCase(name);
    ...
    private PlantDefinition findPlantIgnoringCase(
            String plantName
    ) {
    ...
    findSunProducerZombieDefinition() {
        ...
        ZombieDefinition strongestArmoredZombie =
                null;
    ...
    private void fillMissingZombieDefinitions(
        ...
        List<ZombieDefinition> allDefinitions =
                new ArrayList<>();
    ...
    private boolean isSelectableZombie(
            ZombieDefinition definition
    ) {
    ...
    loadBundledDefinitions() {
        try {
            PlantDefinitionRepository plants =
                    CsvPlantDefinitionRepository.fromClasspath(
                            PLANTS_RESOURCE
                    );
