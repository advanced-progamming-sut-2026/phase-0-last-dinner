...
public class PlantZombieWallnutBowlingIntegration
    ...
    private PlantZombieWallnutBowlingIntegration(
    ...
    public PlantZombieWallnutBowlingIntegration(
        ...
        if (plantDefinitions == null
            ...
            throw new IllegalArgumentException(
                    "Wallnut Bowling requires plant "
    ...
    public void prepareStage(int stageNumber) {
        ...
        ZombieDefinitionRepository stageRepository =
                new StageZombieRepository(
                        chooseStageZombieDefinitions(stageNumber)
                );
    ...
    public void startZombieWaves(int stageNumber) {
        ...
        waveManager.configureWaves(
                createWaves(stageNumber)
        );
    }
    ...
    public int getCherryBombDamage() {
        PlantDefinition cherryBomb =
                findPlantIgnoringCase("Cherry Bomb");

        if (cherryBomb == null) {
            return DEFAULT_CHERRY_BOMB_DAMAGE;
        }

        int damage = DamageExpressionParser.parseTotalDamage(
                cherryBomb.getDamageExpression()
        );
    ...
    public boolean hasZombieAt(Position position) {
        ...
        for (Zombie zombie
                : board.getZombiesAt(boardPosition)) {
    ...
    public void damageFirstZombieAt(
        ...
        if (zombie != null) {
            combatSystem.applyDamageToZombie(
                    zombie,
                    damage
            );
        }
    ...
    public boolean areAllWavesFinished() {
        if (!isReady()
                || waveManager.getWaves() == null
                || waveManager.getWaves().isEmpty()) {

            return false;
        }

        int finalWaveIndex =
                waveManager.getWaves().size() - 1;

        if (waveManager.getCurrentWaveIndex()
                != finalWaveIndex) {
        ...
        return finalWave != null
                && finalWave.isStarted()
                && finalWave
                .getRemainingHealthPercentage() <= 0;
    }
    ...
    private List<ZombieDefinition>
    chooseStageZombieDefinitions(
            int stageNumber
    ) {
    ...
    private boolean isStageOneZombie(
    ...
    private Zombie findFirstZombieAt(
    ...
    private boolean isAliveEnemyZombie(
            Zombie zombie
    ) {
        return zombie != null
                && !zombie.isDead()
    ...
    private ZombieDefinition findZombieByAliases(
            String... aliases
    ) {
    ...
    loadBundledDefinitions() {
        try {
            ...
            throw new IllegalStateException(
                    "Could not load Wallnut Bowling data.",
                    exception
            );
        }
    }
