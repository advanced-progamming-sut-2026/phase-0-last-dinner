...
public class Board {
    ...
    public void addZombie(Zombie zombie, Position position) {
        if (zombie == null || position == null) return;
    ...
    public boolean moveZombie(Zombie zombie, Position destination) {
        if (zombie == null || destination == null) return false;
        if (destination.getX() < 0) return this.handleZombieAtHouse(zombie);
        ...
        if (sourceTile != null) sourceTile.removeZombie(zombie);
    ...
    public LawnMower getLawnMower(int row) {
        for (LawnMower lawnMower : this.lawnMowers) {
            if (lawnMower != null && lawnMower.getRow() == row) return lawnMower;
        }
        return null;
    }
    ...
    public boolean removeZombie(Zombie zombie) {
        if (zombie == null || zombie.getPosition() == null) return false;
        Tile tile = this.getTile(zombie.getPosition());
        if (tile == null || !tile.removeZombie(zombie)) return false;
        zombie.setBoard(null);
        return true;
    }
    public boolean movePlant(Plant plant, Position destination) {
        if (plant == null || destination == null) return false;
        Tile destinationTile = this.getTile(destination);
        if (destinationTile == null || !destinationTile.canPlacePlant(plant)) return false;
        Tile sourceTile = this.getTile(plant.getPosition());
        if (sourceTile != null) sourceTile.removePlant(plant);
    ...
    public List<Plant> getPlantsAt(Position position) {
        ...
        if (position == null) return plantsAtPosition;
        Tile tile = this.getTile(position);
        if (tile == null || tile.getPlants() == null) return plantsAtPosition;
    ...
    public List<Zombie> getZombiesInRadius(Position center, int radius) {
        ...
        if (center == null || this.tiles == null) return zombiesInRadius;
    ...
    public List<Zombie> getZombiesAt(Position position) {
        ...
        if (position == null) return zombiesAtPosition;
        Tile tile = this.getTile(position);
    ...
    public boolean hasLivingZombies() {
        for (Zombie zombie : this.getAllZombies()) {
    ...
    public boolean setTerrain(Position position, TerrainType terrainType) {
        ...
        for (int i = 0; i < this.tiles.size(); i++) {
            ...
            if (tile.getPosition().getX() == position.getX()
                    && tile.getPosition().getY() == position.getY()) {
                boolean graveDestroyed = tile.getTerrainType() == TerrainType.GRAVE
                        && terrainType != TerrainType.GRAVE;
                tile.setTerrainType(terrainType);
                if (graveDestroyed) {
                    this.collectGraveLoot(tile);
                }
                return true;
            }
        }
    ...
    public GraveLootType collectGraveLoot(Tile tile) {
        if (tile == null || tile.getTerrainType() == TerrainType.GRAVE) {
            return GraveLootType.NONE;
        }
        GraveLootType loot = tile.collectGraveLoot();
        if (loot == GraveLootType.SUN && this.sunSystem != null) {
            this.sunSystem.addSun(50);
            this.fireEvent("The grave dropped 50 suns.");
        } else if (loot == GraveLootType.PLANT_FOOD && this.plantFoodSystem != null) {
            this.plantFoodSystem.addPlantFood();
        }
        return loot;
    }
