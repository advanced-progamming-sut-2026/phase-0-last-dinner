...
public final class IZombieScreen implements Screen {
    ...
    public IZombieScreen(Main application, IZombieMiniGame game, int stageNumber, Runnable onBack) {
        ...
        IZombieActionResult startResult = this.controller.onStartIZombieRequested(stageNumber);

        if (startResult == null || !startResult.isSuccessful())
            throw new IllegalStateException(startResult == null ? "Could not start I Zombie." : startResult.getMessage());
    ...
    private void restartGame() {
        ...
        switchScreen(new IZombieScreen(this.application, this.game, this.stageNumber, this.onBack));
    }
