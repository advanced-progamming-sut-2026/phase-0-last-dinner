...
public final class PlantDetailsPanel extends Group {
    ...
    private static final float PREVIEW_WIDTH = 460f;
    private static final float PREVIEW_HEIGHT = 420f;
    ...
    private static final Color INFO_YELLOW = new Color(1f, 0.84f, 0.18f, 1f);
    ...
    public PlantDetailsPanel(Skin skin, GameAssetManager assets, PamAnimationCatalog animationCatalog) {
        ...
        this.abilityValue = label("", 0.50f, Align.topLeft);
        this.levelEffectsValue = label("", 0.44f, Align.topLeft);
    ...
    private void addPreviewPanel() {
        ...
        panel.setBounds(245f, 285f, 540f, 565f);
        addAt(panel, colorImage(PREVIEW_BORDER), 0f, 0f, 540f, 565f);
        addPreviewBackground(panel);
        this.previewVisual.setBounds(40f, 88f, PREVIEW_WIDTH, PREVIEW_HEIGHT);
        panel.addActor(this.previewVisual);
        this.levelLabel.setBounds(80f, 42f, 300f, 45f);
        panel.addActor(this.levelLabel);
        this.lockLabel.setBounds(378f, 44f, 130f, 42f);
        panel.addActor(this.lockLabel);
        addActor(panel);

        Image xpIcon = image(XP_ICON);
        xpIcon.setBounds(270f, 230f, 38f, 38f);
        addActor(xpIcon);
        this.seedBar.setBounds(315f, 236f, 430f, 27f);
        addActor(this.seedBar);
        this.seedLabel.setBounds(315f, 234f, 430f, 30f);
        addActor(this.seedLabel);
    }

    private void addPreviewBackground(Group panel) {
        TextureRegion region = this.textureBank.region(PREVIEW_BACKGROUND);
        if (region != null) {
            addAt(panel, new Image(new TextureRegionDrawable(region)), 12f, 96f, 516f, 457f);
            return;
        }
        addAt(panel, colorImage(PREVIEW_FALLBACK), 12f, 96f, 516f, 457f);
    }

    private void addStats() {
        addStat(SUN_BACKGROUND, "SUN COST", this.sunCostValue, 880f, 725f, true);
        addStat(RECHARGE_ICON, "RECHARGE", this.rechargeValue, 1280f, 725f, false);
        addStat(TOUGHNESS_ICON, "TOUGHNESS", this.healthValue, 880f, 600f, false);
        addStat(DAMAGE_ICON, "DAMAGE", this.damageValue, 1280f, 600f, false);
        addStat(VARIABLE_ICON, "ACTION", this.actionValue, 880f, 475f, false);
        addStat(SPECIAL_ICON, "SPECIAL", this.specialValue, 1280f, 475f, false);

        this.familyBadge.setBounds(880f, 345f, 72f, 72f);
        addActor(this.familyBadge);
        addSmallHeading("FAMILY", 968f, 385f, 220f);
        this.familyValue.setBounds(968f, 343f, 300f, 52f);
        addActor(this.familyValue);

        Image plantFood = image(PLANT_FOOD_ICON);
        plantFood.setBounds(1280f, 342f, 72f, 72f);
        addActor(plantFood);
        addSmallHeading("PLANT FOOD", 1365f, 385f, 270f);
        this.plantFoodValue.setWrap(true);
        this.plantFoodValue.setBounds(1365f, 300f, 430f, 94f);
        addActor(this.plantFoodValue);

        addSmallHeading("ABILITY", 880f, 245f, 230f);
        this.abilityValue.setWrap(true);
        this.abilityValue.setBounds(880f, 130f, 915f, 112f);
        addActor(this.abilityValue);
        this.abilityValue.setColor(Color.WHITE);

        this.levelEffectsValue.setWrap(true);
        this.levelEffectsValue.setBounds(880f, 76f, 915f, 48f);
    ...
    private void addStat(String iconId, String heading, Label value, float x, float y, boolean sunIcon) {
        if (sunIcon) {
            Group sun = new Group();
            sun.setBounds(x, y, 74f, 74f);
            addAt(sun, image(iconId), 7f, 7f, 60f, 60f);
            addAt(sun, image(SUN_GLYPH), 16f, 15f, 43f, 44f);
            addActor(sun);
        } else {
            Image icon = image(iconId);
            icon.setBounds(x + 7f, y + 7f, 60f, 60f);
            addActor(icon);
        }
        addSmallHeading(heading, x + 88f, y + 42f, 245f);
        value.setBounds(x + 88f, y - 2f, 300f, 52f);
        addActor(value);
    }

    private void addSmallHeading(String text, float x, float y, float width) {
        Label heading = label(text, 0.48f, Align.left);
        heading.setBounds(x, y, width, 32f);
        addActor(heading);
    }
    ...
    private void addCollectionActionButton() {
        this.collectionActionButton.setBounds(330f, 125f, 300f, 78f);
        addActor(this.collectionActionButton);
    }

    private void updateCollectionAction(PlantCollectionState state) {
        if (!state.isUnlocked()) {
            this.collectionActionLabel.setText("UNLOCK  2,000");
            this.collectionActionButton.setDisabled(false);
            this.collectionActionButton.setVisible(true);
            return;
        }
        if (isUpgradeAvailable(state)) {
            this.collectionActionLabel.setText("UPGRADE  " + formatCoins(state.getRequiredCoins()));
            this.collectionActionButton.setDisabled(false);
            this.collectionActionButton.setVisible(true);
            return;
        }
        this.collectionActionLabel.setText("FIND MORE");
        this.collectionActionButton.setDisabled(true);
        this.collectionActionButton.setVisible(true);
    }
    ...
    private Actor createStaticPlant(PlantCollectionState state) {
        ...
        float fitScale = Math.min(350f / region.getRegionWidth(), 330f / region.getRegionHeight());
