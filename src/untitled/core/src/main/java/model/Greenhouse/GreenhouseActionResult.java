...
public class GreenhouseActionResult {
    ...
    private GreenhouseActionResult(
        ...
        this.coinsSpent = Math.max(0, coinsSpent);
    ...
    public static GreenhouseActionResult unlocked(
        Position position,
        int coinsSpent,
        int remainingCoins,
        int remainingDiamonds
        ...
        return new GreenhouseActionResult(
            GreenhouseActionStatus.POT_UNLOCKED,
            "Pot at " + position + " was unlocked. "
                + coinsSpent + " coins were spent.",
            position,
            null,
            0,
            coinsSpent,
    ...
    public static GreenhouseActionResult failure(
        ...
        if (status == GreenhouseActionStatus.PLANTED
            ...
            || status == GreenhouseActionStatus.POT_UNLOCKED
            || status
            == GreenhouseActionStatus.GROWTH_ACCELERATED) {
    ...
    public boolean isSuccessful() {
        return this.status == GreenhouseActionStatus.PLANTED
            || this.status == GreenhouseActionStatus.HARVESTED
            || this.status == GreenhouseActionStatus.POT_UNLOCKED
            || this.status
            == GreenhouseActionStatus.GROWTH_ACCELERATED;
    }
}
