...
public class PlantZombieZombotanyIntegration implements ZombotanyIntegration {
    ...
    private final Map<Zombie, ZombotanyTrait> assignedZombieTraits;
    private Board board;
    ...
    private PlantZombieZombotanyIntegration(BundledDefinitions definitions) {
    ...
    public PlantZombieZombotanyIntegration(
        ...
        if (plantDefinitions == null || zombieDefinitions == null) {
            throw new IllegalArgumentException("Zombotany requires plant and zombie definitions.");
        }
        ...
        this.assignedZombieTraits = new IdentityHashMap<>();
    ...
    @Override
    public void prepareStage(ZombotanyStageConfig stageConfig) {
        if (stageConfig == null) {
            throw new IllegalArgumentException("Zombotany stage configuration cannot be null.");
        }
        ...
        initializeZombieWaves(stageConfig);
        addStageStartingSun(stageConfig.getStageNumber());
    }

    private void initializeBoardSystems() {
        this.board = new Board();
        this.engine = new GameEngine(board);
        this.sunSystem = new SunSystem(board, engine.getClock());
        this.plantFoodSystem = new PlantFoodSystem(board);
        this.cooldownManager = new PlantCooldownManager(engine.getClock());
        this.plantingSystem = new PlantingSystem(board, sunSystem, cooldownManager);
        this.combatSystem = new CombatSystem(board);
    }

    private void initializeZombieWaves(ZombotanyStageConfig stageConfig) {
        ZombieDefinitionRepository filteredRepository =
            new FilteredZombieDefinitionRepository(availableZombies);
        this.zombieSpawner = new ZombieSpawner(zombieFactory, filteredRepository, board);
        this.waveManager = new WaveManager(createWaves(stageConfig), zombieSpawner);
    ...
    private void addStageStartingSun(int stageNumber) {
        if (stageNumber == 2) {
            sunSystem.addSun(25);
    ...
    public boolean plant(String plantName, Position position) {
        if (!isReady() || plantName == null || plantName.trim().isEmpty()) {
            return false;
        }
        ...
        PlantDefinition definition = findAvailablePlant(plantName);
        if (definition == null) {
            return false;
        }
        Plant plant = plantFactory.create(definition);
        if (plant == null || !plantingSystem.canPlant(plant, boardPosition)) {
            return false;
        }
        plantingSystem.plant(plant, boardPosition);
    ...
    public boolean usePlantFood(Position position) {
        ...
        return plantFoodSystem.feedPlant(boardPosition);
    }

    @Override
    public void advanceOneTick() {
        if (!isReady() || !engine.isGameRunning()) {
            return;
        }
    ...
    public int getSunAmount() {
        return sunSystem == null ? 0 : sunSystem.getSunAmount();
    }

    @Override
    public int getPlantFoodAmount() {
        return plantFoodSystem == null ? 0 : plantFoodSystem.getPlantFoodAmount();
    }

    @Override
    public int getCurrentWaveNumber() {
        if (waveManager == null || waveManager.getCurrentWave() == null) {
            return 0;
        }
        return waveManager.getCurrentWave().getNumber();
    }

    @Override
    public int getWaveCount() {
        return currentConfig == null ? 0 : currentConfig.getWaveCount();
    }
    ...
    public boolean areAllWavesFinished() {
        if (!isReady() || waveManager.getCurrentWave() == null) {
            return false;
        }
        Wave currentWave = waveManager.getCurrentWave();
        return currentWave.isFinalWave() && !waveManager.hasNextWave() && getAliveZombieCount() == 0;
    }
    ...
    public List<PlantDefinition> getAvailablePlants() {
        return Collections.unmodifiableList(new ArrayList<>(availablePlants));
    }
    ...
    @Override
    public Map<ZombieDefinition, ZombotanyTrait> getZombieTraits() {
        return Collections.unmodifiableMap(new LinkedHashMap<>(zombieTraits));
    }
    ...
    public ZombotanyTrait getTrait(Zombie zombie) {
        ...
        ZombotanyTrait assignedTrait = assignedZombieTraits.get(zombie);
        if (assignedTrait != null) {
            return assignedTrait;
        }
        return zombieTraits.get(zombie.getDefinition());
    }
    ...
    private void configureAvailablePlants(ZombotanyStageConfig stageConfig) {
        availablePlants = new ArrayList<>();
        for (String plantName : stageConfig.getAvailablePlantNames()) {
            PlantDefinition definition = findPlantDefinition(plantName);
            if (definition != null && !availablePlants.contains(definition)) {
        ...
        if (availablePlants.isEmpty()) {
            throw new IllegalStateException("No plant definitions are available for Zombotany.");
        }
    }

    private void configureZombieTraits(ZombotanyStageConfig stageConfig) {
        zombieTraits = new LinkedHashMap<>();
        availableZombies = new ArrayList<>();
        for (ZombotanyTrait trait : stageConfig.getAvailableTraits()) {
            ZombieDefinition definition = findPreferredZombieForTrait(trait);
            if (definition == null || availableZombies.contains(definition)) {
                definition = findUnusedZombieDefinition();
            }
            if (definition == null) {
                continue;
            }
        ...
        if (availableZombies.isEmpty()) {
            throw new IllegalStateException("No zombie definitions are available for Zombotany.");
        }
    }

    private ZombieDefinition findPreferredZombieForTrait(ZombotanyTrait trait) {
        if (trait == null) {
            return null;
        }
        String[] preferredAliases = getPreferredAliases(trait);
        for (String alias : preferredAliases) {
            ZombieDefinition definition = zombieDefinitions.findByAlias(alias);
            if (isUsableZombieDefinition(definition) && !availableZombies.contains(definition)) {
                return definition;
            }
        }
        return null;
    ...
    private static String[] getPreferredAliases(ZombotanyTrait trait) {
        switch (trait) {
            case PEASHOOTER:
                return new String[]{"ZombieDefault", "ZombieTutorialDefault"};
            case WALLNUT:
                return new String[]{"ZombieArmor2", "ZombieTutorialArmor2"};
            case JALAPENO:
                return new String[]{"ZombieArmor1", "ZombieTutorialArmor1"};
            case SQUASH:
                return new String[]{"ZombieArmor4", "ZombieModernAllStar"};
            default:
                return new String[0];
        }
    }
    ...
    private ZombieDefinition findUnusedZombieDefinition() {
        List<ZombieDefinition> definitions = zombieDefinitions.findAll();
        if (definitions == null) {
            return null;
        }
    ...
    private boolean isUsableZombieDefinition(ZombieDefinition definition) {
        return definition != null && definition.getWavePointCost() > 0 && definition.getHitpoints() > 0;
    }

    private List<Wave> createWaves(ZombotanyStageConfig stageConfig) {
        List<Wave> waves = new ArrayList<>();
        List<Integer> difficulties = stageConfig.getWaveDifficulties();
        for (int index = 0; index < difficulties.size(); index++) {
            boolean finalWave = index == difficulties.size() - 1;
            waves.add(new Wave(index + 1, difficulties.get(index) * 100.0, finalWave));
        }
        return waves;
    }
    ...
    private void applyTraitsToNewZombies() {
        ...
        for (Zombie zombie : board.getAllZombies()) {
            if (zombie == null
                || zombie.isDead()
                || zombie.getWave() == null
                || assignedZombieTraits.containsKey(zombie)) {
                continue;
            }
            ZombotanyTrait trait = zombieTraits.get(zombie.getDefinition());
            if (trait == null) {
                continue;
            }
            ZombieBehavior behavior = createTraitBehavior(trait);
            if (behavior == null) {
                continue;
            }
            zombie.addBehavior(behavior);
            assignedZombieTraits.put(zombie, trait);
        }
    }

    private ZombieBehavior createTraitBehavior(ZombotanyTrait trait) {
        if (trait == null) {
            return null;
        }
        switch (trait) {
            case PEASHOOTER:
                return new ZombotanyPeashooterBehavior();
            case WALLNUT:
                return new ZombotanyWallnutBehavior();
            case JALAPENO:
                return new ZombotanyJalapenoBehavior();
            case SQUASH:
                return new ZombotanySquashBehavior();
            default:
                return null;
        }
    }

    private PlantDefinition findAvailablePlant(String name) {
        if (name == null) {
            return null;
        }
        for (PlantDefinition definition : availablePlants) {
            if (definition != null
                && definition.getName() != null
                && definition.getName().equalsIgnoreCase(name.trim())) {
                return definition;
            }
        }
        return null;
    ...
    private PlantDefinition findPlantDefinition(String name) {
        if (name == null) {
            return null;
        }
        PlantDefinition exact = plantDefinitions.findByName(name);
        if (exact != null) {
            return exact;
        }
        for (PlantDefinition definition : plantDefinitions.findAll()) {
    ...
    public long getRemainingCooldownTicks(String plantName) {
        ...
        if (plant == null || this.cooldownManager == null)
            return 0L;

        return this.cooldownManager.getRemainingTicks(plant);
    }

    @Override
    public Wave getCurrentWave() {
        return this.waveManager == null ? null : this.waveManager.getCurrentWave();
    }

    private Position toBoardPosition(Position externalPosition) {
    ...
    private static BundledDefinitions loadBundledDefinitions() {
        try {
            ...
            throw new IllegalStateException("Could not load bundled Zombotany definitions.", exception);
        }
    }
    ...
    private static class FilteredZombieDefinitionRepository implements ZombieDefinitionRepository {
        ...
        private FilteredZombieDefinitionRepository(List<ZombieDefinition> definitions) {
        ...
        public ZombieDefinition findByAlias(String alias) {
            ...
            for (ZombieDefinition definition : definitions) {
                if (definition != null
                    ...
                    && definition.getAlias().equalsIgnoreCase(alias)) {
                    return definition;
                }
            }
            return null;
