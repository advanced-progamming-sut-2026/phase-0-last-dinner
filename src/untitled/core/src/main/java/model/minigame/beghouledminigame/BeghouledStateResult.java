...
public class BeghouledStateResult {
  ...
  public BeghouledStateResult(
    ...
    this.highestUnlockedStage = Math.max(1, highestUnlockedStage);

    this.sunAmount = Math.max(0, sunAmount);
    this.completedMatchCount = Math.max(0, completedMatchCount);
    this.targetMatchCount = Math.max(0, targetMatchCount);

    this.availablePlantTypes =
        availablePlantTypes == null
            ? Collections.emptyList()
            : Collections.unmodifiableList(new ArrayList<>(availablePlantTypes));

    this.upgradeOptions =
        upgradeOptions == null
            ? Collections.emptyList()
            : Collections.unmodifiableList(new ArrayList<>(upgradeOptions));
    ...
    this.craters =
        craters == null
            ...
            : Collections.unmodifiableSet(new HashSet<>(craters));
  ...
  private List<List<String>> copyGrid(List<List<String>> source) {
    if (source == null) {
  ...
  public String getPlantNameAt(Position position) {
    ...
    if (rowIndex < 0 || rowIndex >= plantNames.size()) {
      return "";
    }
  ...
  public boolean isCrater(Position position) {
    return position != null && craters.contains(position);
  }
