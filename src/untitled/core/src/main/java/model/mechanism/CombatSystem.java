...
public class CombatSystem implements Tickable {
    ...
    // in field ha etelaate kill ro baraye emtiaz negah midaran
    private GameClock gameClock;
    private ZombieKillObserver killObserver;
    private Projectile currentKillProjectile;
    private Plant currentKillPlant;
    private Runnable plantDestroyedObserver;
    private boolean coldImmuneZombies;
    public CombatSystem() {
    ...
    public CombatSystem(Board board, LootSystem lootSystem) {
        ...
        if (board != null) board.setCombatSystem(this);
    }
    public CombatSystem(Board board, GameEngine gameEngine, LootSystem lootSystem, PlantFoodSystem plantFoodSystem) {
        ...
        if (board != null && plantFoodSystem != null) board.setPlantFoodSystem(plantFoodSystem);
    }
    @Override
    public void onTick() {
    ...
    public void attack(Zombie zombie) {
        if (zombie == null || zombie.isDead() || zombie.getPosition() == null || this.board == null) return;
        Plant target = this.board.getNearestPlantInZombieAttackRange(zombie.getPosition(), 1);
        if (target != null) zombie.attack(target);
    }
    public void applyDamageToZombie(Zombie zombie, int damage) {
        this.applyDamageToZombie(zombie, damage, null);
    }
    public void applyDamageToZombie(Zombie zombie, int damage, Plant sourcePlant) {
        if (!this.canTakePlantDamage(zombie, damage) || this.isSubmerged(zombie)) return;
        Plant previousSource = this.currentKillPlant;
        this.currentKillPlant = sourcePlant;
        try {
            zombie.takeDamage(damage);
            this.finishZombieDeath(zombie);
        } finally {
            this.currentKillPlant = previousSource;
        }
    }
    public void applyDirectDamageToZombie(Zombie zombie, int damage) {
        this.applyDirectDamageToZombie(zombie, damage, null);
    }
    public void applyDirectDamageToZombie(Zombie zombie, int damage, Plant sourcePlant) {
        if (!this.canTakePlantDamage(zombie, damage) || this.isSubmerged(zombie)) return;
        Plant previousSource = this.currentKillPlant;
        this.currentKillPlant = sourcePlant;
        try {
    ...
    public void applyDamageToPlant(Plant plant, int damage) {
        if (plant == null || damage <= 0 || plant.isDead()) return;
        plant.takeDamage(damage);
        if (plant.isDead()) {
            plant.activateUpgradeDeathEffects(this.board);
            this.firePlantDestroyedEvent(plant);
            if (this.plantDestroyedObserver != null) this.plantDestroyedObserver.run();
            this.removePlantFromBoard(plant);
        }
    }
    public void destroyPlant(Plant plant) {
        if (plant != null) this.applyDamageToPlant(plant, Math.max(1, plant.getHealth()));
    }
    public void killZombie(Zombie zombie) {
        this.killZombie(zombie, null);
    }
    public void killZombie(Zombie zombie, Plant sourcePlant) {
        if (zombie == null || zombie.isHypnotized() || this.isSubmerged(zombie)) return;
        this.killZombieIgnoringAllegiance(zombie, sourcePlant);
    }
    ...
    public void killZombieIgnoringAllegiance(Zombie zombie) {
        this.killZombieIgnoringAllegiance(zombie, null);
    }
    public void killZombieIgnoringAllegiance(Zombie zombie, Plant sourcePlant) {
        if (zombie == null) {
            return;
        }
        Plant previousSource = this.currentKillPlant;
    ...
    private void resolveProjectiles() {
        ...
        while (iterator.hasNext()) {
            ...
            if (projectile == null || this.resolveProjectile(projectile)) {
                iterator.remove();
            }
        }
    }
    private boolean resolveProjectile(Projectile projectile) {
        ...
        if (this.interceptProjectile(projectile)) {
            return true;
        }
    ...
    private boolean interceptProjectile(Projectile projectile) {
        ...
        TerrainType terrainBeforeHit = tile == null ? null : tile.getTerrainType();
        if (tile != null && tile.intercept(projectile)) {
            if (terrainBeforeHit == TerrainType.GRAVE && tile.getTerrainType() != TerrainType.GRAVE) {
                this.board.collectGraveLoot(tile);
            }
            return true;
        }
    ...
    private boolean resolveZombieProjectileHit(Projectile projectile) {
        ...
        if (behavior != null && behavior.onProjectileHit(target, projectile, this.board)) {
            return !projectile.isHostileToPlants();
        }
    ...
    private void applyPoisonDamage() {
        for (Zombie zombie : this.board.getAllZombies()) {
            ...
            this.applyDirectDamageToZombie(
                    zombie,
                    zombie.getPoisonDamagePerTick(),
                    zombie.getPoisonSourcePlant()
            );
        }
    }
    ...
    private void applyProjectileHit(Projectile projectile, Zombie directTarget) {
        for (Zombie target : this.getProjectileAffectedTargets(projectile, directTarget)) {
            ...
            this.currentKillProjectile = projectile;
            try {
                ...
                this.currentKillProjectile = null;
            }
    ...
    private void applyProjectileConditions(Projectile projectile, Zombie zombie) {
        ...
        boolean blockedByChapter = this.coldImmuneZombies
                && (condition == ZombieCondition.CHILLED || condition == ZombieCondition.FROZEN);
        if (condition != null && !blockedByChapter) {
        ...
        if (projectile.getPoisonDamagePerTick() > 0) {
            ...
            if (zombie.hasCondition(ZombieCondition.POISONED)) {
                ...
                zombie.setPoisonSourcePlant(projectile.getSourcePlant());
            }
        }
    ...
    private void finishZombieDeath(Zombie zombie) {
        ...
        if (this.killObserver != null) {
            long tick = this.gameClock == null ? 0 : this.gameClock.getCurrentTick();
            this.killObserver.onZombieKilled(
                    new ZombieKillEvent(
                            zombie,
                            this.currentKillProjectile,
                            tick,
                            this.currentKillPlant != null
                                    ? this.currentKillPlant
                                    : this.currentKillProjectile == null
                                            ? null
                                            : this.currentKillProjectile.getSourcePlant()
                    )
            );
        }
        Loot loot = this.lootSystem.generateZombieDrop(zombie);
        this.lootSystem.collectLoot(loot);
